...
- A page-backed copy-on-write package in [`pagebtree/`](pagebtree/) using slotted pages, linked leaves, overflow pages, growable/compactable mmap-backed storage, offline mmap copy compaction, experimental sparse-hole punching for safe mmap free pages, read-only mmap reader slots, stale-reader cleanup, tunable kernel page-cache advice, bounded branch-routing cache, validation audit reports, cache residency stats, and optional mmap trace events with JSONL export
