...
// bytes visible to the old root. Current-tree Range uses those leaf links when
// no active reader can make them stale, and mmap-backed ranges prefetch a small
// window of exact next leaf pages with MADV_WILLNEED. Mmap-backed trees track
// dirty copied pages so Sync can flush changed data pages before publishing
// metadata, and they can grow the mapped file when allocation reaches the
// current capacity.
