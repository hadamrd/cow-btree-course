...
`MmapSpaceStats` is the matching physical-space observer for sparse-file work. On Unix it reads `stat(2)` data from the mapped database file and reports logical file bytes, mapped bytes, allocated bytes, sparse bytes, allocated filesystem blocks, the 512-byte block unit used by the allocation count, the filesystem's preferred I/O block size, and the filesystem identity when the platform exposes one. Linux reports numeric filesystem magic through `statfs(2)` and derives mount path, source, and mount options from `/proc/self/mountinfo`; Darwin and FreeBSD report filesystem type name, mount path, and mount source from `statfs(2)`. `MmapHolePunchProfile()` complements those counters with the platform-level sparse-punch capability used by this build. Both are intentionally evidence surfaces rather than correctness invariants: allocation can be rounded up, compressed, cloned, delayed, or unsupported depending on the filesystem. The APIs are still useful because they let experiments compare `Compact`, `CopyCompactMmap`, `CompactMmapFile`, and `PunchFreeMmapPages` with stable measurement and capability fields.
...
- `mmap-reader-table-cleanup` when stale dead or detectably reused reader-table slots are explicitly cleared
...
optimistic conflicts, can mix deterministic staged deletes into successful
transactions with `--delete-every N`, reopens the database to verify which
staged writes survived and which staged deletes stayed deleted, and optionally
writes value-free trace JSONL. `--redact-path` omits local database and trace
paths from the JSON report while still writing the trace file at the supplied
path:
...
`InspectMmapReaderStats(path)` is different from `OpenMmapReadOnly(...).MmapReaderStats()`: it maps the database read-only long enough to run the same checked metadata and root-page recovery used by normal open, opens an already-existing `.readers` file without initializing it, and scans the table without claiming a slot. Use it for offline or command-line observation when the measurement itself should not hold back page recycling. If the newest metadata record is checksum-valid but its root page is torn, this passive inspection validates reader slots against the older recovered root revision, not against the rejected newer metadata revision.
