...
- insertion and delete redistribution choose byte-aware leaf and branch split points, byte-full leaf rewrites can spill cells to overflow pages, and leaf repair can trigger on low byte occupancy at minimum key count; `Stats` reports reachable page byte fill, but branch repair is still triggered by key count rather than byte occupancy
