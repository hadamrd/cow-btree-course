...
- insertion and delete redistribution choose byte-aware leaf and branch split points, byte-full leaf rewrites can spill cells to overflow pages, and leaf/branch repair can trigger on low byte occupancy at minimum key count; `Stats` reports reachable page byte fill, but merge-versus-redistribute choice is still conservative rather than a full byte-occupancy policy
