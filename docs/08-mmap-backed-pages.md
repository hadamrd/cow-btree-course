...
The key-order field is intentionally modest: it records that branch separators and leaf slots are sorted by unsigned bytewise lexicographic comparison. The zero value is accepted as bytewise for older images, but new metadata writes the explicit `KeyOrderBytewise` identifier. An unknown requested order or an unknown persisted order is rejected before the tree walks branch child pointers. Memory-backed trees can install a custom `KeyComparator`, but mmap-backed trees reject custom comparators today because the file format does not yet persist a durable comparator identity. This compatibility promise is backed by `pagebtree/testdata/mmap-v2-legacy-zero-key-order.db`, a real mmap image with zero key-order metadata. The current mmap boundary is therefore explicit: bytewise is durable; custom comparators need a future persisted identity before mmap can safely reopen them.
