...
- insertion and delete redistribution choose byte-aware leaf and branch split points, and byte-full leaf rewrites can spill cells to overflow pages; `Stats` reports reachable page byte fill, but underfull repair is still triggered by key count rather than byte occupancy
