...
`MmapSpaceStats` is the matching physical-space observer for sparse-file work. On Unix it reads `stat(2)` data from the mapped database file and reports logical file bytes, mapped bytes, allocated bytes, sparse bytes, allocated filesystem blocks, the 512-byte block unit used by the allocation count, and the filesystem's preferred I/O block size. This is intentionally filesystem-reported evidence rather than a correctness invariant: allocation can be rounded up, compressed, cloned, delayed, or unsupported depending on the filesystem. The API is still useful because it lets experiments compare `Compact`, `CopyCompactMmap`, `CompactMmapFile`, and `PunchFreeMmapPages` with a stable measurement surface.
...
and `--space` adds `stat(2)` logical-vs-allocated file-space counts for sparse
experiments. `--pages` adds value-free page summaries with role, kind, byte
