...
- insertion chooses byte-aware leaf and branch split points and byte-full leaf rewrites can spill cells to overflow pages; `Stats` reports reachable page byte fill, but key-underfull leaves and branches still merge or redistribute by key count rather than byte-balanced occupancy
