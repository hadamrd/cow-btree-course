...
- insertion chooses byte-aware leaf and branch split points, leaf delete redistribution is byte-aware, and byte-full leaf rewrites can spill cells to overflow pages; `Stats` reports reachable page byte fill, but underfull branches still merge or redistribute by key count rather than byte-balanced occupancy
