...
- Insertion and delete redistribution choose byte-aware leaf and branch split points, byte-full leaf rewrites spill inline cells to overflow pages, leaf/branch repair can trigger on low byte occupancy at minimum key count, and merges require combined bytes to fit in one page.
