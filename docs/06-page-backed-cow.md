...
Current-tree `Range` uses the leaf chain when no active reader can make those links stale. `RangeFrom(start)` first descends the tree to the leaf that can contain `start`, then scans forward through linked leaves and skips entries below the lower bound. `RangeBetween(start, end)` uses the same lower-bound leaf descent, stops before the exclusive `end` key, and avoids prefetching linked leaves whose first key is already outside the bound. `CursorBetween(start, end)` gives the same half-open interval shape to incremental seek/next users while keeping the snapshot pin owned by the cursor; `Last` and `Prev` let that same bounded cursor walk backward without materializing the interval. A tree-owned cursor can also `Delete` its current key from the live tree. That delete publishes a new copy-on-write root, while the cursor remains positioned on its old snapshot and can continue scanning. Inside each leaf, these scans walk slot entries directly instead of materializing every key/value cell first. If a reader is active, these methods fall back to the recursive branch walk so they still return the current keys even while link repair is deferred; that fallback also reads branch child ids directly from slots only for children it actually visits. Snapshot ranges and snapshot cursors preserve the old-root view directly.
