...
| Gap | Why it matters | Current state | Next useful slice |
| --- | --- | --- | --- |
| Byte-balanced pages | Variable-size records make key-count balancing weak. | Started: `Stats` now reports reachable leaf/branch/overflow page counts plus total and per-kind used bytes, free bytes, and capacity; leaf insertion chooses a legal split point by encoded cell byte footprint, with overflow fallback for large inline values. Branch splits and delete redistribution are still mostly degree/key-count based. | Extend byte occupancy policy to branch split choices and delete redistribution. |
