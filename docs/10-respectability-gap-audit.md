...
| Transaction batching | Real engines commit a unit of work, not one implicit root publish per call. | Advanced: `WriteBatch` stages point `Put`/`Delete` operations and half-open `DeleteRange`, hides them until `Commit`, and publishes one tree revision across memory and mmap trees. `BeginReadWrite` adds a small transaction facade with read-your-writes `Get`, transaction-visible `RangeBetween`, range delete expansion over the staged view, rollback, byte-key wrappers, and commit through the same batch machinery. `CommitDetailed` reports per-operation old values, returns explicit invalid-commit errors, and restores the pre-commit tree state if staged replay panics before publication. `Sync` remains the mmap durability boundary. Tree-owned cursor delete is point-wise and still publishes immediately. | Add cursor-write semantics inside explicit transactions plus a fuller ACID isolation/durability contract. |
