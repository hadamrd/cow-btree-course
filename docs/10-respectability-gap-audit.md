...
| Recycling | Serious but narrow | In-process readers and mmap reader-table watermarks keep retired pages out of the free list until old readers are gone; reclaim metadata survives reopen; version-3 reader slots add process-start and boot/session owner tags where available while keeping v1/v2 sidecars readable; a subprocess lifecycle test now proves a real child read-only handle pins retired pages until it exits. | Long-running multi-process soak testing and owner-token coverage on more Unix targets remain open. |
