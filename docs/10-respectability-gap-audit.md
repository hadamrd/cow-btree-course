...
| Gap | Why it matters | Current state | Next useful slice |
| --- | --- | --- | --- |
| Byte-balanced pages | Variable-size records make key-count balancing weak. | Started: `Stats` now reports reachable leaf/branch/overflow page counts plus total and per-kind used bytes, free bytes, and capacity; insertion and delete redistribution choose legal leaf/branch split points by encoded cell byte footprint, large inline values can spill to overflow pages, and leaf repair has a conservative low-byte trigger at the minimum key count. Branch repair and merge decisions are still mostly degree/key-count based. | Extend byte-occupancy thresholds to branch repair and merge/redistribution choice. |
