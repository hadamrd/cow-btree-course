...
| Multi-process robustness | Reader tables need stronger owner identity than PID alone. | Version-2 slots use PID, revision, token, and process-start token where available; stale cleanup treats a live PID with a mismatched process-start token as stale, while version-1 sidecars remain readable as untagged slots. | Add boot/session identity, platform-specific start-token implementations beyond `/proc`, and multi-process soak testing. |
