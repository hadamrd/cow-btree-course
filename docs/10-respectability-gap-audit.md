...
| Operations | Useful lab tools | `mmapinspect`, trace hooks, cache residency stats, compact-copy commands, bounded reader-table soak runner, torn-image lab command, and benchmarks exist. | No release process, no compatibility matrix, no corruption corpus management, and no production support envelope. |
...
| Durable metadata evolution | Format upgrades need explicit compatibility tests. | Started: metadata versions are decoded behind checksum validation; `pagebtree/testdata/mmap-v2-legacy-zero-key-order.db` covers pre-key-order metadata, and `pagebtree/testdata/mmap-v1-inline-freelist.db` plus `TestMmapTreeOpensLegacyV1InlineFreelistFixture` cover a version-1 inline freelist image and subsequent page reuse. | Add more old-format fixtures, especially chained legacy freelists and future upgrade/rewrite tests. |
