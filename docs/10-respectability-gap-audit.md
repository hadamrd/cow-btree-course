...
| Gap | Why it matters | Current state | Next useful slice |
| --- | --- | --- | --- |
| Byte-balanced pages | Variable-size records make key-count balancing weak. | Started: `Stats` now reports reachable leaf/branch/overflow page counts plus total and per-kind used bytes, free bytes, and capacity; insertion chooses legal leaf and branch split points by encoded cell byte footprint, with overflow fallback for large inline values. Delete redistribution is still mostly degree/key-count based. | Extend byte occupancy policy to delete redistribution. |
