...
| Recycling | Serious but narrow | In-process readers and mmap reader-table watermarks keep retired pages out of the free list until old readers are gone; reclaim metadata survives reopen; version-2 reader slots add process-start owner tags where available. | Boot/session identity and long-running multi-process operational cases still need harder owner identity and soak testing. |
...
| Multi-process robustness | Reader tables need stronger owner identity than PID alone. | Version-2 slots use PID, revision, token, and process-start token where available; Linux reads the token from `/proc`, Darwin reads it with `sysctl`, stale cleanup treats a live PID with a mismatched process-start token as stale, version-1 sidecars remain readable as untagged slots, and `MmapReaderStats` reports format version plus token-support state. | Add boot/session identity, start-token implementations for other Unix targets, and multi-process soak testing. |
