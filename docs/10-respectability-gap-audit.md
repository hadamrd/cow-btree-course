...
- Experimental sparse-hole punching for already-free mmap page extents on
  ...
  filters and trace events for attempted ranges, aggregate results, and
  failures.
...
- Opt-in mmap trace events for sync phases and failures, recovery candidate
  ...
  freelist/reclaim metadata rollback, sparse-hole punching, stale reader
...
- Process-exit crash probes for direct sync publication, write-batch
  commit-sync publication, transaction commit-sync publication, transaction
  commit followed by mmap sync publication, legacy metadata upgrade
  publication, and obsolete freelist metadata-generation reclaim.
...
- `AuditReport` to expose validation evidence: current stats, reachable page
  IDs, reusable page IDs, retired page IDs, persisted metadata page IDs,
  value-free page summaries,
...
| mmap durability | Respectable local harness, not a product guarantee | Dirty data pages are synced before metadata publication, dual metadata pages are checked, process-exit crash probes classify old-root/new-root recovery, and `cmd/mmaptearlab` simulates torn newest metadata/root images with JSON fallback evidence. | Still lacks real power-fail rigs, filesystem-specific evidence, and broad torn-sector campaigns. |
| Recycling | Serious but narrow | In-process readers and mmap reader-table watermarks keep retired pages out of the free list until old readers are gone; reclaim metadata survives reopen; version-3 reader slots add process-start and boot/session owner tags where available while keeping v1/v2 sidecars readable; subprocess tests now prove real child read-only handles pin retired pages until they exit, including a four-reader mutation stress case; `cmd/mmapreadersoak` turns that pattern into a bounded JSON-reporting local soak runner. | Long-duration external soak labs and owner-token coverage on more Unix targets remain open. |
...
| Operations | Useful lab tools | `mmapinspect`, trace hooks, cache residency stats, compact-copy commands, bounded reader-table soak runner, torn-image lab command, local filesystem probe command, and benchmarks exist. | No release process, no compatibility matrix, no corruption corpus management, and no production support envelope. |
...
| Process-exit crash probes exist, but are still not power-fail proof | [`pagebtree/mmap_process_crash_test.go`](../pagebtree/mmap_process_crash_test.go) |
...
| Local filesystem space behavior can be probed on a disposable path | [`cmd/mmapfsprobe/main.go`](../cmd/mmapfsprobe/main.go), [`cmd/mmapfsprobe/main_test.go`](../cmd/mmapfsprobe/main_test.go) |
...
| --- | --- | --- | --- |
| Crash fault injection | Recovery code is only respectable when tested at every publish boundary. | Started: the internal rollback matrix covers sync, growth, and compact-shrink boundaries. The copied-image crash harness now classifies sync-publication, growth, compact-shrink, large-freelist spill, large-reclaim spill, and obsolete metadata-generation reclaim images. `TestMmapSyncProcessCrashMatrixClassifiesRecoveryRoot` kills a child writer at sync-publication fault points and reopens the same database from a fresh process. `TestMmapWriteBatchCommitSyncProcessCrashMatrixClassifiesRecoveryRoot` stages a multi-operation write batch through `CommitSyncDetailed`, kills the child writer inside the sync boundary, and proves old-root recovery before dirty data sync plus new-root recovery after metadata bytes are written. `TestMmapReadWriteTxCommitSyncProcessCrashMatrixClassifiesRecoveryRoot` does the same through transaction `CommitSyncDetailed`, proving the one-call transaction commit-sync helper has the same recovery boundary. `TestMmapTxSyncProcessCrashMatrixClassifiesRecoveryRoot` covers transaction commit followed by a separate mmap `Sync`. `TestMmapGrowthProcessCrashMatrixClassifiesOldRoot` does the same for growth file-size, directory-sync, and pre-remap fault points, all recovering the old root. `TestMmapCompactShrinkProcessCrashMatrixClassifiesCompactedRoot` does the same for compact-shrink file-size, directory-sync, and pre-remap fault points, reopening live keys from the compacted root. `TestMmapLargeFreelistProcessCrashMatrixClassifiesRecoveryRoot` does the same for large-freelist spill publication, preserving old metadata before data sync and persisted reusable-page records after metadata write. `TestMmapLargeReclaimProcessCrashMatrixClassifiesReaderPinnedRetiredPages` keeps a live read-only parent handle while killing the child writer at large-reclaim publication points, proving persisted retired records remain pinned by a surviving reader watermark. `TestMmapLegacyMetadataUpgradeProcessCrashMatrixClassifiesRecoveryRoot` kills a child while upgrading a real version-1 metadata fixture, proving before-data-sync recovery keeps the old upgrade-pending metadata and post-meta-write recovery accepts the version-2 alternate slot. `TestMmapObsoleteFreelistProcessCrashClassifiesMetadataGenerationReclaim` kills a child after the second and third large-freelist syncs, proving old freelist metadata pages are not reusable while a valid metadata slot can still name them and are reusable once both slots have advanced. `cmd/mmaptearlab` adds a runnable torn-image simulation for newest metadata and newest root-page damage. | True power-fail testing, broader torn-sector campaigns, and filesystem-specific fsync matrices are still outside the local harness. |
| Transaction batching | Real engines commit a unit of work, not one implicit root publish per call. | Advanced: `WriteBatch` stages point `Put`/`Delete` operations and half-open `DeleteRange`, hides them until `Commit`, and publishes one tree revision across memory and mmap trees. `BeginReadWrite` adds a small transaction facade with stable begin-revision reads, read-your-writes `Get`, transaction-visible `RangeBetween`, transaction cursors with staged `Delete`, range delete expansion over the staged view, rollback, byte-key wrappers, optimistic revision-conflict detection through `ErrTxConflict`, and commit through the same batch machinery. `CommitDetailed` reports per-operation old values, returns explicit invalid-commit errors, and restores the pre-commit tree state if staged replay panics before publication. `CommitSync` and `CommitSyncDetailed` now add explicit commit-then-sync helpers for batches and read-write transactions; if sync fails, the detailed result still reports the logical commit while the error says durable publication is not proven. Batch and transaction fault tests prove that retrying `Sync` after before-data-sync, after-metadata-write, and before-metadata-sync failures can publish that already-visible commit and survive close/reopen. `Sync` remains the mmap durability boundary; process-crash tests now verify both write-batch and read-write-transaction `CommitSyncDetailed`, plus transaction commit followed by separate `Sync`, recover as old-root before dirty data sync and new-root after metadata bytes are written. `TestMmapReadWriteTxCommitSyncDetailedConflictIsObservableAbort` proves a conflicted transaction `CommitSyncDetailed` emits `mmap-tx-conflict`, starts no mmap sync, preserves `SyncedRevision`, discards staged writes, and reopens from the previously synced root. `cmd/mmaptxworkload` adds a bounded fresh-database workload that alternates transaction commit-sync success with forced optimistic conflicts, can mix deterministic staged deletes into successful transactions, can hold same-process read-only mmap handles or separate read-only child processes to expose reader-pinned reclaim pressure, verifies reopened committed/deleted/conflicted key counts, accepts a stable publishable report label, and can export the value-free trace for `cmd/mmaptracesummary`. `docs/txworkloads` now contains redacted same-process and child-process reader samples plus a generated summary verified by `scripts/verify-tx-workload-artifacts.sh`. Tree-owned cursor delete outside a transaction is still point-wise and publishes immediately. | Add a fuller ACID contract: true concurrent workload stress, process-crash matrices around heavier conflict workloads, and documented fsync guarantees per filesystem. |
...
| Sparse-file punching | Reusable interior pages should not always keep physical blocks allocated. | Started: `PunchFreeMmapPages` coalesces safe free page IDs into page-aligned ranges, skips pages still reachable from either valid metadata slot, respects reader-pinned retirement by reclaiming only safe retired pages first, preserves the freelist and file length, and uses Linux `fallocate(PUNCH_HOLE|KEEP_SIZE)` when available. `MmapHolePunchProfile` exposes the build's supported primitive or unsupported reason, `mmapinspect --space` prints it next to `MmapSpaceStats`, `cmd/mmappunch` runs active sparse-hole maintenance with before/after logical-versus-allocated file-space evidence plus optional JSONL trace export, and `cmd/mmapfsprobe` creates a disposable local workload that records insert/delete/compact/punch space phases for the actual filesystem path with an optional publishable report label and path redaction mode for shareable JSON. `cmd/fsprobesummary` converts saved probe JSON from one or more filesystems into a stable Markdown comparison table and prefers that label over the raw path. `docs/probes` now contains one redacted Darwin/APFS sample plus a generated summary verified by `scripts/verify-probe-artifacts.sh`. `MmapSpaceStats` now also reports filesystem and mount identity where the platform exposes it, so probe output can distinguish APFS/tmpfs/ext4/XFS-like environments and mount options instead of only byte counts. `MmapOptions.TraceHook` emits `mmap-punch-begin`, per-range `mmap-punch-range`, aggregate `mmap-punch-end`, and `mmap-punch-failed` events with skipped fallback-recoverable pages, attempted ranges, punched page/byte counts, and failure reasons. | Record broader filesystem matrix runs and add platform-specific implementations beyond Linux. |
| Multi-process robustness | Reader tables need stronger owner identity than PID alone. | Version-3 slots use PID, revision, token, process-start token, and boot/session token where available; Linux reads process start from `/proc/<pid>/stat` and boot identity from `/proc/sys/kernel/random/boot_id`, Darwin reads process start from `kern.proc.pid` and boot identity from `kern.boottime`, stale cleanup treats a live PID with mismatched owner tokens as stale, v1/v2 sidecars remain readable as older formats, `MmapReaderStats` reports format version plus process-start/boot token support and slot counts, `InspectMmapReaderStats` can scan an existing sidecar without claiming a reader slot and validates slots against the same recovered usable root revision normal open would accept, `TestMmapReadOnlyChildProcessPinsRecyclingUntilRelease` covers a separate PID opening `OpenMmapReadOnly` while a writer mutates and later reclaims, `TestMmapReadOnlyChildProcessesPinRecyclingAcrossMutations` covers four child reader processes across repeated writer mutations and staggered release, and `cmd/mmapreadersoak` provides a configurable local command for bounded reader-process soak runs. | Add owner-token implementations for more Unix targets and long-duration soak runs outside `go test`. |
| Observability | A serious engine should explain stalls, reclaim pressure, and recovery decisions. | Started: `MmapOptions.TraceHook` emits structured value-free `MmapTraceEvent` records for sync phases, timed per-range dirty data-page flushes, sync failures, transaction conflicts, recovery candidate accept/reject decisions, growth and compact remap success/failure geometry, sparse-hole punch begin/range/end/failure decisions, freelist/reclaim metadata rollback page spans, stale reader cleanup counts, and obsolete metadata-page reclaim decisions. `MmapTraceJSONLExporter`, `MmapTraceAsyncJSONLExporter`, and `cmd/mmaptrace-demo` provide workload JSONL export paths for experiments, with bounded async drop accounting and docs that call out the redaction boundary if applications add their own key/value fields. `cmd/mmaptracesummary` turns trace JSONL into a stable Markdown review artifact with event-kind counts, dirty-range totals, sparse-punch totals, transaction-conflict totals and reasons, failure reasons, and a bounded timeline. `cmd/mmaptxsummary` turns transaction workload JSON reports into a stable Markdown table covering commit/delete/conflict counts, reopened-key verification, synced revision, and reader-pinned reclaim pressure; its readers cell distinguishes same-process handles from child process readers as `local+process/observed` and prefers stable labels over local file paths for shareable artifacts. `Stats` exposes counters, `Stats.SyncedRevision` distinguishes logical versus successfully synced revision, and `Stats.ReclaimPressure` separates reader-pinned retired pages from immediately reclaimable retired pages with the oldest reader watermark. `AuditReport` exposes validation traversal evidence, reachable/free/retired page IDs, persisted freelist/reclaim metadata page IDs, and value-free page summaries with reclaim revision bounds. `InspectMmapRecovery` and `cmd/mmapinspect` expose accepted/rejected metadata recovery candidates for fallback diagnosis without requiring a separate workload trace file; `mmapinspect` also emits invalid JSON with recovery candidates when normal read-only open fails, includes passive reader stats or reader-table errors on that failed-open path when `--readers` is requested, and can include passive writer-sidecar lock diagnostics when `--locks` is requested. `cmd/mmapinspect` also prints read-only audit JSON including numeric and named key-order/comparator identity plus optional non-intrusive reader-table stats, writer lock stats, cache-residency, file-space, page-summary, bounded key-sample, JSONL trace-correlation, a bounded ordered phase timeline, dirty-range, and sparse-punch summary sections. `MmapCacheStats` exposes kernel residency, and `MmapSpaceStats` exposes filesystem allocation evidence. | Add workload dashboards or external observability adapters. |
...
- `cmd/mmapfsprobe` creates a disposable mmap database and reports local
  insert/delete/compact/punch logical-versus-allocated space evidence for the
  actual filesystem path, including filesystem and mount identity when
  available.
- `cmd/fsprobesummary` turns one or more saved `mmapfsprobe` JSON reports into
  a stable Markdown comparison table for review notes and manual platform
  matrices.
