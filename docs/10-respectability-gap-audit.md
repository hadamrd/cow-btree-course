...
| Gap | Why it matters | Current state | Next useful slice |
| --- | --- | --- | --- |
| Byte-balanced pages | Variable-size records make key-count balancing weak. | Started: `Stats` now reports reachable leaf/branch/overflow page counts plus total and per-kind used bytes, free bytes, and capacity; insertion and delete redistribution choose legal leaf/branch split points by encoded cell byte footprint, large inline values can spill to overflow pages, and leaf/branch repair have conservative low-byte triggers at the minimum key count. Merge decisions are still mostly degree/key-count based. | Extend byte-occupancy policy to the choice between merge and redistribution. |
