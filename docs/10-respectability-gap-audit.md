...
pattern to large-reclaim paths and eventually running true power-fail probes
outside the local harness. The large-freelist process-exit matrix already covers
old-metadata recovery before data sync and persisted freelist reuse after
metadata write or before metadata sync.
